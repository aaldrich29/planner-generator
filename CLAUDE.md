...
- `andrewsFavoriteItems()` / `addAndrewsFavorite()` / `printAndrewsFavorite()` — the "★ Andrew's Favorite" one-click week pack in the queue panel: 5 (M–F) or 7 (M–Su) daily slips with margin left and dated notes on the back, then the weekly review (margin left) and the *next* week's weekly plan (margin right). Every front carries an explicit back, so the page count is even and two-sided printing pairs review + plan on one sheet; it forces the Sides toggle to `1` so the app doesn't insert its own blank backs.
