...
- `andrewsFavoriteItems()` / `addAndrewsFavorite()` / `printAndrewsFavorite()` — the "★ Andrew's Favorite" one-click week pack in the queue panel: 5 (M–F) or 7 (M–Su) daily slips with dated notes on the back, then the weekly review and the *next* week's weekly plan. The slips and the review are right-hand pages (margin mode `right` — the default padding, wide margin on the **left**); the plan is a left-hand page (`left` → wide margin on the right). Every front carries an explicit back, so the page count is even and two-sided printing pairs review + plan on one sheet; it forces the Sides toggle to `1` so the app doesn't insert its own blank backs.
